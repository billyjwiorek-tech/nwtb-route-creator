(() => {
  ...
  async function refreshVisitMap(){try{const j=await call('visit_list');visitMap.clear();(j.statuses||[]).forEach(x=>visitMap.set(x.account_key,x.status));if(Array.isArray(accounts))accounts.forEach(a=>a.visitStatus=visitMap.get(accountKey(a))||'NOT_VISITED')}catch(e){console.warn('Visit status load failed',e)}}
  ...
  if(typeof oldBuild==='function')window.buildRoute=async function(){await refreshVisitMap();const t=document.getElementById('routeType')?.value;const includeFollow=document.getElementById('includeFollowUps')?.checked;const saved=accounts.map(a=>({a,routeEligible:a.routeEligible}));accounts.forEach(a=>{if(a.broadType==='PROSPECT'){const s=visitMap.get(accountKey(a))||'NOT_VISITED';a.visitStatus=s;if(s==='DO_NOT_ROUTE'||s==='NOT_A_FIT')a.routeEligible=false;else if((t==='new_business'||t==='prospects_regular')&&s==='FOLLOW_UP'&&!includeFollow)a.routeEligible=false;else if((t==='new_business'||t==='prospects_regular')&&s!=='NOT_VISITED'&&s!=='FOLLOW_UP')a.routeEligible=false}});try{return await oldBuild()}finally{saved.forEach(x=>x.a.routeEligible=x.routeEligible)}};
