...
const NEAR_STOP_M=152.4,ARRIVAL_M=6.096,ARRIVAL_SOFT_MAX_M=30;
...
function updateDisplay(pos,forceSpeak=false,gps={}){
 ...
 const step=currentStep();if(step){const loc=step?.maneuver?.location||[],mpos=loc.length===2?{lat:Number(loc[1]),lon:Number(loc[0])}:navTarget;let d=hav(pos,mpos);
  ...
  const now=currentStep(),txt=instruction(now);document.getElementById('nwtbTurn').textContent=txt;document.getElementById('nwtbTurnDist').textContent=feet(d);const speed=Math.max(6,Number(window.__nwtbLastSpeedMps||0)||13.4);document.getElementById('nwtbEta').textContent=eta(remaining/speed);if(forceSpeak||d<220)speak(`${d<45?'Now, ':''}${txt}`)
 }
 document.getElementById('nwtbRemain').textContent=feet(remaining);updateNearGlow(pos,remaining);
 if(exact||arrivalHits>=2){document.getElementById('nwtbNavBanner').textContent=`ARRIVED: ${navTarget.name}`;if(navTarget.stop_id)showArrival();else speak(`You have arrived at ${navTarget.name}`)}else document.getElementById('nwtbNavBanner').textContent=remaining<=NEAR_STOP_M&&navTarget.stop_id?`NEAR STOP: ${navTarget.name}`:`Navigating to ${navTarget.name}`
}
...
async function beginNavigation(target,routeId){navTarget=target;navRouteId=routeId||route||'';navLastSpoken='';navStepIndex=0;arrivalFor='';arrivalHits=0;hideArrival();clearNearGlow();const p=ensurePanel();p.classList.add('show');document.getElementById('nwtbNavBanner').className='navBanner';document.getElementById('nwtbNavBanner').textContent='Getting your location…';if(!navigator.geolocation){document.getElementById('nwtbNavBanner').className='navBanner warn';document.getElementById('nwtbNavBanner').textContent='This phone/browser does not provide GPS.';return}navigator.geolocation.getCurrentPosition(async g=>{const pos={lat:g.coords.latitude,lon:g.coords.longitude};initMap(pos.lat,pos.lon);await reroute(pos,true);startNavWatch();try{if(typeof requestWakeLock==='function')requestWakeLock()}catch{}},e=>{document.getElementById('nwtbNavBanner').className='navBanner warn';document.getElementById('nwtbNavBanner').textContent='Allow Location access to use NWTB navigation: '+e.message},{enableHighAccuracy:true,maximumAge:3000,timeout:15000})}
...
window.nwtbStopNavigation=()=>{if(navWatch!==null){navigator.geolocation.clearWatch(navWatch);navWatch=null}clearNearGlow();navTarget=null;navSteps=[];navRouteCoords=[];navStepIndex=0;arrivalFor='';arrivalHits=0;hideArrival();try{speechSynthesis.cancel()}catch{}const p=document.getElementById('nwtbNavPanel');if(p)p.classList.remove('show')};
