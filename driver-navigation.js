...
let navMap=null,navLine=null,navPos=null,navDest=null,navWatch=null,navTarget=null,navSteps=[],navStepIndex=0,navRouteCoords=[],navLastReroute=0,navLastSpoken='',navRouteId='',navRouteMeta=null,baseFinish=null,arrivalFor='';
...
const feet=m=>m<160.934?`${Math.max(20,Math.round(m/10)*10)} ft`:`${(m/1609.344).toFixed(1)} mi`;
...
function drawRoute(current){if(!navMap||!window.L)return;if(navLine)navMap.removeLayer(navLine);if(navRouteCoords.length){const ll=navRouteCoords.map(c=>[c[1],c[0]]);navLine=L.polyline(ll,{weight:7,opacity:.8}).addTo(navMap);const b=navLine.getBounds();if(b.isValid())navMap.fitBounds(b,{padding:[28,28],maxZoom:16})}if(navPos)navMap.removeLayer(navPos);navPos=L.circleMarker([current.lat,current.lon],{radius:9,weight:4,fillOpacity:1}).addTo(navMap).bindTooltip('You',{permanent:false});if(navDest)navMap.removeLayer(navDest);navDest=L.marker([navTarget.lat,navTarget.lon]).addTo(navMap).bindPopup(navTarget.name||'Destination')}
...
function updateDisplay(pos,forceSpeak=false){
 ...
 if(remaining<45){document.getElementById('nwtbNavBanner').textContent=`ARRIVING: ${navTarget.name}`;if(navTarget.stop_id)showArrival();else speak(`You have arrived at ${navTarget.name}`)}else document.getElementById('nwtbNavBanner').textContent=`Navigating to ${navTarget.name}`
}
...
async function beginNavigation(target,routeId){navTarget=target;navRouteId=routeId||route||'';navLastSpoken='';navStepIndex=0;arrivalFor='';hideArrival();const p=ensurePanel();p.classList.add('show');document.getElementById('nwtbNavBanner').className='navBanner';document.getElementById('nwtbNavBanner').textContent='Getting your location…';if(!navigator.geolocation){document.getElementById('nwtbNavBanner').className='navBanner warn';document.getElementById('nwtbNavBanner').textContent='This phone/browser does not provide GPS.';return}navigator.geolocation.getCurrentPosition(async g=>{const pos={lat:g.coords.latitude,lon:g.coords.longitude};initMap(pos.lat,pos.lon);await reroute(pos,true);startNavWatch();try{if(typeof requestWakeLock==='function')requestWakeLock()}catch{}},e=>{document.getElementById('nwtbNavBanner').className='navBanner warn';document.getElementById('nwtbNavBanner').textContent='Allow Location access to use NWTB navigation: '+e.message},{enableHighAccuracy:true,maximumAge:3000,timeout:15000})}
...
window.nwtbCompleteNext=async()=>{if(!navTarget?.stop_id||!baseFinish)return;const id=navTarget.stop_id;hideArrival();try{await baseFinish(id,'DELIVERED');await new Promise(r=>setTimeout(r,250));await navigateNext(navRouteId)}catch(e){alert(e.message||e)}};
window.nwtbNotCompleted=async()=>{if(!navTarget?.stop_id||!baseFinish)return;const id=navTarget.stop_id;hideArrival();try{await baseFinish(id,'NOT_DELIVERED');await new Promise(r=>setTimeout(r,250));await navigateNext(navRouteId)}catch(e){alert(e.message||e)}};
...
window.nwtbStopNavigation=()=>{if(navWatch!==null){navigator.geolocation.clearWatch(navWatch);navWatch=null}navTarget=null;navSteps=[];navRouteCoords=[];navStepIndex=0;arrivalFor='';hideArrival();try{speechSynthesis.cancel()}catch{}const p=document.getElementById('nwtbNavPanel');if(p)p.classList.remove('show')};
