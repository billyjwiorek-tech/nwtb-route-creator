...
function install(){
 ...
 .nwtbDriverVan{position:relative;transform-origin:50% 58%;filter:drop-shadow(0 3px 5px rgba(15,23,42,.38));will-change:transform}.nwtbDriverVan.desktop{width:82px;height:54px}.nwtbDriverVan.mobile{width:58px;height:40px}
 .nwtbDriverVan .body{position:absolute;left:5px;top:9px;width:60px;height:34px;background:linear-gradient(180deg,#fff,#eef2f6);border:2.5px solid #17202a;border-radius:7px 5px 5px 7px;overflow:hidden}.nwtbDriverVan.mobile .body{left:4px;top:8px;width:40px;height:24px;border-width:2px}
 .nwtbDriverVan .cab{position:absolute;left:56px;top:16px;width:22px;height:25px;background:#fff;border:2.5px solid #17202a;border-left:0;border-radius:0 8px 5px 0}.nwtbDriverVan.mobile .cab{left:40px;top:13px;width:15px;height:19px;border-width:2px;border-left:0;border-radius:0 6px 4px 0}
 .nwtbDriverVan .window{position:absolute;left:4px;top:4px;width:11px;height:8px;background:#bfdbfe;border:1.5px solid #17202a;border-radius:2px}.nwtbDriverVan.mobile .window{left:3px;top:3px;width:8px;height:6px;border-width:1px}
 .nwtbDriverVan .wheel{position:absolute;width:12px;height:12px;border-radius:50%;background:#17202a;border:2px solid #fff;top:38px}.nwtbDriverVan.mobile .wheel{width:9px;height:9px;top:29px;border-width:1.5px}.nwtbDriverVan .w1{left:14px}.nwtbDriverVan .w2{left:59px}.nwtbDriverVan.mobile .w1{left:10px}.nwtbDriverVan.mobile .w2{left:42px}
 ...
 function icon(angle){addStyle();const m=mobile(),w=m?66:94,h=m?50:66,cls=m?'mobile':'desktop';return L.divIcon({className:'',html:`<div class="nwtbDriverVan ${cls}" style="transform:rotate(${angle||0}deg)"><div class="body"><img class="logo" src="nwt-logo.svg" alt="NWTB"><span class="text">NWTB</span></div><div class="cab"><span class="window"></span></div><span class="wheel w1"></span><span class="wheel w2"></span><span class="dot"></span></div>`,iconSize:[w,h],iconAnchor:[w/2,h/2]})}
 L.circleMarker=function(latlng,opts={}){
  ...
  let angle=0,prev=L.latLng(latlng);const marker=L.marker(latlng,{icon:icon(angle),zIndexOffset:1200});const nativeSet=marker.setLatLng.bind(marker);
  marker.setLatLng=function(next){const n=L.latLng(next);if(meters(prev,n)>=3){angle=bearing(prev,n);this.setIcon(icon(angle))}prev=n;return nativeSet(n)};
  return marker
 };
 return true
}
